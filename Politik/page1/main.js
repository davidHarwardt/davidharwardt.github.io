...
Shader.fromFile(gl, "../includes/shaders/basic.vert", "../includes/shaders/fog.frag", (ex, program) =>
{
    ...
    Loader.loadText("../includes/hme.obj", (ex, status, response) =>
    {
        Shader.fromFile(gl, "../includes/shaders/ortho.vert", "../includes/shaders/fog.frag", (ex, orthoProgr) =>
        {
            hmeMesh = Mesh.fromObj(gl, response, orthoProgr, 1 / 40);
...
function draw(ev)
{
    ...
    hmeMesh.draw(gl, new Vector(-0.5, 0.5).add(mousePos.multScalar(SENS).mult(new Vector(1, -1))));
